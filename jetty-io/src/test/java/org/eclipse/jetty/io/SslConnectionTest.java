...
public class SslConnectionTest
{
    ...
    private static class TestEP extends SelectChannelEndPoint
    {
        ...
        {
            System.err.printf("onIncompleteFlush%n");
        ...
        public boolean flush(ByteBuffer... buffers) throws IOException
        {
            if (__startBlocking.get()==0 || __startBlocking.decrementAndGet()==0)
            {
                ...
                {
                    System.err.printf("BLOCK %s + %d%n",BufferUtil.toDetailString(buffers[0]),buffers.length-1);
                    return false;
                }
            }
            String s=BufferUtil.toDetailString(buffers[0]);
            boolean flushed=super.flush(buffers);
            System.err.printf("flush=%b %s + %d%n",flushed,s,buffers.length-1);
            return flushed;
        }
